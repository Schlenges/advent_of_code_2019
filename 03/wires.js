...
const getCoordinates = (input) => {
  ...
  input.map(instruction => {
    ...
    let z = parseInt(instruction.substring(1))
...
const getRange = (a, b) => {
  let all = []
  if(a.x == b.x){
    let z = Math.min(a.y, b.y)
    for(let i = 1; i <= Math.abs(a.y - b.y); i++){
      all.push([a.x, z+i])
    }
  }

  if(a.y == b.y){
    let z = Math.min(a.x, b.x)
    for(let i = 1; i < Math.abs(a.x - b.x); i++){
      all.push([z+i, a.y])
    }
  }
...
console.log(findIntersection(wire1, wire2))